...
from config_distributed import get_default_config, ModelConfig, get_weights_file_path, get_latest_weights_file_path
...
def greedy_decode(model: nn.Module, source: torch.Tensor, source_mask: torch.Tensor, tokenizer_src: Tokenizer, tokenizer_tgt: Tokenizer, max_len: int, device: torch.device):
    ...
    # model is isntance of DistributedDataParallel and from that we acces module
...
def get_ds(config: ModelConfig):
    ds_raw = load_dataset(config.dataset,f"{config.lang_src}-{config.lang_tgt}", split='train')
     # Build tokenizers
...
if __name__ == '__main__':
    ...
    # Support both single-GPU (python) and distributed (torchrun) modes
    ...
    if config.local_rank == 0:
        ...
        if not is_distributed:
            print("\n[INFO] Running in single-GPU mode. Use 'torchrun' for distributed training.")

    if is_distributed:
        init_process_group()
    else:
        # For single-GPU, initialize with a single-process group
        init_process_group(backend='nccl', init_method='tcp://localhost:12355', rank=0, world_size=1)
