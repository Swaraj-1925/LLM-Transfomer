...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        enc_num_pad = self.seq_len - len(enc_input_token) - 2
        dec_num_pad = self.seq_len - len(dec_input_token) - 1
        ...
        encoder_input = torch.cat(
                [
                    ...
                    self.eos_token,
                    torch.tensor(
                        [self.pad_token] * enc_num_pad, dtype=torch.int64
                        )
                ]
                )
        ...
        decoder_input = torch.cat(
                [
                    ...
                    torch.tensor(
                        [self.pad_token] * dec_num_pad, dtype=torch.int64
                        )
                ]
                )
